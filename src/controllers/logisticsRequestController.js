...
const logisticsRequestController = {
    ...
    async createRequest(req, res, next) {
        try {
            const { requester, equipment, product, pickupLocation, deliveryLocation, scheduledPickupDate, scheduledDeliveryDate, status } = req.body;
            const request = new LogisticsRequest({ requester, equipment, product, pickupLocation, deliveryLocation, scheduledPickupDate, scheduledDeliveryDate, status });
            await request.save();
    ...
    async updateRequest(req, res, next) {
        try {
            ...
            const request = await LogisticsRequest.findByIdAndUpdate(
                req.params.id,
                { requester, equipment, product, pickupLocation, deliveryLocation, scheduledPickupDate, scheduledDeliveryDate, status },
